...
7. **Breakout story generation (scripts)**
   ...
   Stories use the same LLM and optional KB as the app. Each story is tailored to that document’s theme statement and keypoints (no fixed theme list). Participant names are read from the DOCX Stories subsection when present and replace the (Name) placeholder. Output is limited to a configurable word count (default 130) and post-processed to remove repeated sentences and phrases. Results are written to **src/modules/questionnaire/output/questionnaire/** as **<input_docx_stem>_YYYYMMDD_HHMMSS.txt**. See `PRD.md` for scope.
