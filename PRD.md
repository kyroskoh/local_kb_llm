...
- **Breakout story generation**: Parse report DOCX Breakout section into themes, questions, topics, and keypoints; optionally parse participant names from the Stories subsection; generate one narrative story per topic (or one per theme when aggregated). Story content is tailored to the document’s theme statement and keypoints (vectordb/KB not used; training data is for summarization patterns only, not names or locations). When gender is unknown, refer to the participant as (Name) only (no pronouns); when known, use `participant_pronoun="he"` or `"she"` for consistent pronouns. Max-word limit (default 130); post-processing removes repetition and meta-commentary; (Name) replaced by parsed participant name when available. Output is written to **output/<module_name>/** at project root as **<input_docx_stem>_<timestamp>.txt**.
