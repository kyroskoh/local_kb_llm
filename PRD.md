...
- **Questionnaire module**: Summarize filled questionnaires (agreed + background) using module training data; support template as JSON or PDF/DOCX/TXT/EPUB. **Breakout story generation**: Extract themes, questions, topics, and keypoints from the Breakout Group Discussion section of report DOCX; generate short narrative stories per topic (or per theme) using the same LLM and optional training data. Stories are tailored to each document’s theme statement and keypoints (document-agnostic; each DOCX has its own themes and keypoint style). When gender is unknown, refer to the participant as **(Name)** only (no he/she/they); when gender is known, pass `participant_pronoun="he"` or `"she"` for consistent third-person pronouns. Participant names from the DOCX Stories subsection replace (Name) when available. Stories are constrained by max words (default 130) and post-processed to remove repeated sentences/phrases and meta-commentary (e.g. "Critical:", "Opening:"). Generated output is written to **src/modules/questionnaire/output/** under a subfolder by module name (e.g. **output/questionnaire/**) as a .txt file with the same base name as the input DOCX and a timestamp suffix (YYYYMMDD_HHMMSS).
...
- **Breakout story generation**: Parse report DOCX Breakout section into themes, questions, topics, and keypoints; optionally parse participant names from the Stories subsection; generate one narrative story per topic (or one per theme when aggregated). Story content is tailored to the document’s theme statement and keypoints (no fixed theme list; each DOCX may have different themes and keypoint structure). When gender is unknown, refer to the participant as (Name) only (no pronouns); when known, use `participant_pronoun="he"` or `"she"` for consistent pronouns. Max-word limit (default 130); post-processing removes repetition and meta-commentary; (Name) replaced by parsed participant name when available. Output is written to **src/modules/questionnaire/output/<module_name>/** as **<input_docx_stem>_<timestamp>.txt**.
